...
def render():
    ...
    user_info = query_db(f"SELECT * FROM people WHERE ssn = '{user_ssn}';")
    user_first_name = user_info["first_name"].tolist()[0]
    user_role = user_info["role"].tolist()[0]
    ...
    try:
        book_borrowed = query_db(f"SELECT b.title, b.isbn, r.event_date as borrowed_date, r.lid, r.location FROM booking_records r LEFT JOIN books b ON r.isbn = b.isbn WHERE r.ssn = '{user_ssn}' AND r.ret_date IS NULL ;")
        df = book_borrowed
        st.dataframe(df)
    except:
        st.write("You did not borrowed any book.")
